...
window.load = $.getJSON('../wee_mock_data.json', (data) => {
							houseData = data;
...
function getHouses(size, price, location) {
	...
	for (var i = 0, j = 0; i < houseData.length; i++) {
		...
		if (house.size <= size && house.price <= price && (location == '' || location == city)) {
